...
def clean_state_dict_new(state_dict):
    new_state_dict = {}
    for key in state_dict:
        # If we encounter 'weight_orig', apply the mask
        if 'weight_orig' in key:
            ...
            if mask_key in state_dict:
                ...
                # If no mask found, log a warning and use original weights
                new_key = key.replace('weight_orig', 'weight')
...
def main(args):
    ...
    target_folder_name = f'Dataset-MedMNIST-{args.dataset}_LargeScale__Sparsity-{int(args.sparsity * 100)}%__Pruning-{pruning_names[args.pruning_type]}__{current_date}'
    ...
    test_model = SetMLP(input_size, num_classes).to(device)
