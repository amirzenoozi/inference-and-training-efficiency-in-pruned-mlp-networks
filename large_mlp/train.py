...
def main(args):
    ...
    model = SetMLP(input_size[args.dataset], output_size[args.dataset]).to(device)
    ...
    target_folder_name = f'Dataset-{dataset_names[args.dataset]}_LargeScale__Sparsity-{int(args.sparsity * 100)}%__Pruning-{pruning_names[args.pruning_type]}__{current_date}'
    ...
    test_model = SetMLP(input_size[args.dataset], output_size[args.dataset]).to(device)
