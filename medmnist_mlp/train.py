...
def main(args):
    ...
    # Create the datasets
    train_dataset = DataClass(split='train', transform=data_transform, download=True, as_rgb=False, size=None, root=args.data_dir)
    val_dataset = DataClass(split='val', transform=data_transform, download=True, as_rgb=False, size=None, root=args.data_dir)
    test_dataset = DataClass(split='test', transform=data_transform, download=True, as_rgb=False, size=None, root=args.data_dir)
    ...
    model = DynamicSparseMLP(input_size, hidden_size, num_classes).to(device)
    monitor = ZeusMonitor(gpu_indices=[0])

    if args.pruning_type == 'before':
        apply_pruning(model, args.sparsity)
    elif args.pruning_type == 'manual':
        apply_random_pruning(model, args.sparsity)
    elif args.pruning_type == 'erdos':
        erdos_renyi_pruning(model, args.sparsity)

    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    optimizer = Lion(model.parameters(), lr=args.learning_rate, weight_decay=1e-3, betas=(0.9, 0.99))
    # optimizer = optim.SGD(model.parameters(), lr=args.learning_rate, weight_decay=1e-4, momentum=0.9, nesterov=True)
    # optimizer = optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=1e-5, betas=(0.9, 0.999), eps=1e-08, amsgrad=False)

    warmup_epochs = 5
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer,lr_lambda=lambda epoch: min(1.0, epoch / warmup_epochs))
    ...
    target_folder_name = f'Dataset-MedMNIST-{args.dataset}_SmallScale__Sparsity-{int(args.sparsity * 100)}%__Pruning-{pruning_names[args.pruning_type]}__{current_date}'
    ...
    for epoch in range(args.epochs):
        ...
        if args.pruning_type == 'manual' or args.pruning_type == 'erdos':
            modify_weights_mocanu(model, args.sparsity, 0.2)
    ...
    test_model = DynamicSparseMLP(input_size, hidden_size, num_classes).to(device)
    ...
    with torch.no_grad():  # No need to calculate gradients during testing
        for inputs, labels in test_loader:
            ...
            total_inference_energy_list.append(inference_measurement)
            num_batches += 1
    ...
    total_energy_by_zeus = sum(map(lambda m: m.total_energy, total_inference_energy_list))
    total_time_by_zeus = sum(map(lambda m: m.time, total_inference_energy_list))
    avg_energy_by_zeus = total_energy_by_zeus / len(total_inference_energy_list)
    avg_time_by_zeus = total_time_by_zeus / len(total_inference_energy_list)

    test_accuracy = 100 * correct_test / total_test
    print(f'correct_test: {correct_test}, total_test: {total_test}')
