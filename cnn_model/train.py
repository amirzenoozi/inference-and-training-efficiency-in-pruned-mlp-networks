...
def parse_args():
    ...
    parser.add_argument('-K', '--dataset', type=str, default='mnist', help='Choose DataSet Name', choices=['octmnist', 'mnist', 'fmnist', 'emnist', 'cifar10', 'emnist_class', 'cifar100', 'food101'])
    ...
    parser.add_argument('-A', '--arch', type=str, default='vgg16', help='Model architecture used for training: Default is vgg16')
...
def main(args):
    ...
    if args.dataset == 'cifar10':
    ...
    elif args.dataset in ['emnist', 'emnist_class']:
        transform = transforms.Compose([
            transforms.Grayscale(num_output_channels=3),
            transforms.Resize((32, 32)),
            transforms.RandomRotation(0),
            transforms.ToTensor(),
            transforms.Lambda(lambda x: x.transpose(1, 2)),  # transpose to correct orientation
            transforms.Normalize((0.5,), (0.5,))  # grayscale mean & std
        ])
        ...
        transform = transforms.Compose([
            transforms.RandomCrop(32, padding=4),  # Padding + crop for translation invariance
            transforms.RandomHorizontalFlip(),  # Flip for spatial invariance
            transforms.ColorJitter(0.2, 0.2, 0.2, 0.1),  # Light color augmentation
    ...
    if args.dataset == 'mnist':
    ...
    elif args.dataset == 'emnist_class':
        ...
        dataset = datasets.CIFAR100(root=args.data_dir, train=True, download=True, transform=transform)
    elif args.dataset == 'food101':
        dataset = datasets.Food101(root=args.data_dir, train=True, download=True, transform=transform)

    # Split dataset into training and validation sets
    if args.dataset in ['mnist', 'fmnist', 'emnist', 'emnist_class', 'cifar10', 'cifar100', 'food101']:
        ...
        # Create the datasets
        train_dataset = DataClass(split='train', transform=transform, download=True, as_rgb=False, size=None, root=args.data_dir)
        val_dataset = DataClass(split='val', transform=transform, download=True, as_rgb=False, size=None, root=args.data_dir)
        test_dataset = DataClass(split='test', transform=transform, download=True, as_rgb=False, size=None, root=args.data_dir)
    ...
    input_size = {
        ...
        'cifar100': 3,
        'food101': 3,
        'octmnist': in_channel if args.dataset == 'octmnist' else 1,
    }
    hidden_size = 512
    output_size = {
        'mnist': 10,
        ...
        'cifar100': 100,
        'food101': 101,
        'octmnist': num_classes if args.dataset == 'octmnist' else 10,
    }
    img_size = {
        'mnist': 32,
        'fmnist': 32,
    ...
    model = ModifiedCNN(args.arch, input_size[args.dataset], hidden_size, output_size[args.dataset], img_size[args.dataset]).to(device)
    # model = timm.create_model(args.arch, pretrained=True, num_classes=output_size[args.dataset]).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if args.pruning_type == 'manual' or args.pruning_type == 'erdos':
            modify_weights_mocanu(model, args.sparsity, 0.2, False)
        ...
        for inputs, labels in train_loader:
            ...
            all_predictions_train.extend(preds.cpu().numpy())
    ...
    test_model = ModifiedCNN(args.arch, input_size[args.dataset], hidden_size, output_size[args.dataset], img_size[args.dataset]).to(device)
    # test_model = timm.create_model(args.arch, pretrained=True, num_classes=output_size[args.dataset]).to(device)
