...
input_size = 28 * 28
hidden_size = 16
output_size = 10 # The Number of MNIST Dataset Classes
learning_rate = 0.001
batch_size = 4
epochs = 5
...
def main(args):
    ...
    model = SparseMLP(input_size, hidden_size, output_size).to(device)
    # Apply pruning to the first layer of the model
    apply_pruning(model, sparsity_amount)
    ...
    training_info = {
        ...
        'sparsity_amount': sparsity_amount,
    ...
    for epoch in range(args.epochs):
        ...
        if early_stopping.early_stop:
            if 'stopped_at' not in training_info:
